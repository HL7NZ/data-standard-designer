...
There are 2 supporting Use cases

* Pathologist registers receipt of the request - part of the pathologist processing
* View outstanding requests - then have been submitted by a clinician, but not yet processed by the pathologist.

For the purposes of these use cases, it is assumed that there are 5 principal actors.

* A FHIR server that acts as a Forms Manager (holding the Questionnaires)
* A FHIR server that acts as a data server - holding the Patient, Practitioner, Task, QuestionnaireResponse, DiagnosticReport and other supporting resources. This may be the same instance as the Forms Manager.
...
* An application that supports the pathologist - displaying the completed form, inputting the pathologist report, generating the report and saving it to the server
* An application that displays the pathologist report and updates the task

<img style="float:none" src="usecase.png"/>
...
The following resources are included in the bundle, which is POSTed to the server root as described in [FHIR transaction processing](http://hl7.org/fhir/http.html#transaction)

<img style="float:none" src="pathrequest.png"/>
...
<table class = "table table-bordered">
    ...
    <tr>
        <td><a href="http://hl7.org/fhir/patient.html">Patient</a></td>
    ...
    <tr>
        <td><a href="http://hl7.org/fhir/practitioner.html">Practitioner</a></td>
    ...
    <tr>
        <td><a href="http://hl7.org/fhir/organization.html">Organization</a></td>
    ...
    <tr>
        <td><a href="http://hl7.org/fhir/questionnaireresponse.html">QuestionnaireResponse</a></td>
    ...
    <tr>
        <td><a href="http://hl7.org/fhir/servicerequest.html">ServiceRequest</a></td>
        <td>These are the administrative details of the request. </td>
    </tr>
    <tr>
        <td><a href="http://hl7.org/fhir/task.html">Task</a></td>
        <td>Used to track the overall progress of the workflow. This includes the viewing if the report (signed off) by the requesting clinician. In the reference implementation the Task is created and updated by the client apps (form filler and pathology system). In a pilot or production system this could be server responsibility. 
...
<table class='table table-bordered'>
    <tr><th>Source path</th><th>Target</th><th>Description</th></tr>
    <tr>
        <td></td>
        <td></td>
        <td></td>
...
There are no new resources or references created - only the Task is updated. Potentially, the owner could be updated to another resource within the lab as part of internal processing.


### Generate the report

Eventually, the lab will create the report. This will be a combination of DiagnosticReport and Observation resources, depending on the capability of the lab system. There are 2 tasks that the lab must perform:
...
Elements on the Task that are updated:

* the business status is set to 'reportgenerated'
* the owner is set to the clinician that created the initial request

The Task status is *not* updated as the Task remains active.
...
<table class='table table-bordered'>
    ...
    <tr>
        ...
        <td>So that the Task has a reference to the DR </td>
    </tr>
    <tr>
        <td>DR.basedOn</td>
        <td>ServiceRequest</td>
        <td>So that the DR can refer to the SR that caused it to be generated. This is needed
        to find the ordering clinician for example </td>
    </tr>
</table>
